...
document.addEventListener('DOMContentLoaded', () => {
    // Determine the server URL. If running via Vite (port 5173/5174), point expressly to the Node backend on port 3000
    const SERVER_URL = window.location.port.startsWith('517') ? 'http://localhost:3000' : undefined;
    ...
    function renderPlayers(players, hostId) {
        ...
        players.forEach(player => {
            ...
            div.innerHTML = `
                ...
                ${isThisHost ? '<img src="/assets/UI Chalk/host.png" class="host-crown" title="Host">' : ''}
            `;
        ...
        function initStars() {
            ...
            for (let i = 0; i < STARS_COUNT; i++) {
                stars.push({
                    ...
                    vx: Math.random() * 0.1 - 0.05, // very slow horizontal drift
                    vy: Math.random() * 0.1 - 0.05, // very slow vertical drift
                    alpha: Math.random(),
                    dAlpha: (Math.random() - 0.5) * 0.01 // basic twinkle
                });
            }
        }
